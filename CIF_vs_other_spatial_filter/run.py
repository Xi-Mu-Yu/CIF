...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="cif",
        choices=[
            "cif", "car", "bipolar", "laplacian", "csp", "ecg",
            "ica", "ssp", "sss", "linear",
        ],
        help="Channel preprocessing before TCN: cif = front/back blend written to one block (original CIF). "
        "car = common average reference on all channels. bipolar = adjacent bipolar along channel axis. "
        "laplacian = discrete Laplacian (path graph) on all channels. "
        "csp = Common Spatial Patterns: W from training-set trial covariances (min label vs rest), "
        "then x @ W each timestep (fixed, not learned). "
        "ecg = Einthoven leads I–III on first 3 channels (RA,LA,LL order), rest unchanged. "
        "ica = MNE ICA on training trials, drop high-kurtosis components, fixed apply matrix. "
        "ssp = MNE signal-space projection (PCA) removing top-variance spatial modes. "
        "sss = inner subspace reconstruction from training spatial covariance (EEG-compatible SSS analogue). "
        "linear = learnable nn.Linear(enc_in, enc_in, bias=False) channel mixing, end-to-end trained (init: identity, C^2 params).",
    )
    parser.add_argument(
        "--csp_reg",
        type=float,
        default=1e-6,
        help="Tikhonov regularizer on (R1+R2) for CSP generalized eigenproblem.",
    )
    parser.add_argument(
        "--ica_n_components",
        type=int,
        default=None,
        help="ICA components (default: min(enc_in, enc_in-1)). Used when spatial_mode=ica.",
    )
    parser.add_argument(
        "--ica_n_exclude",
        type=int,
        default=1,
        help="Number of ICA components with highest kurtosis to exclude. Used when spatial_mode=ica.",
    )
    parser.add_argument(
        "--ssp_n_proj",
        type=int,
        default=1,
        help="Number of SSP (PCA) projectors to remove. Used when spatial_mode=ssp.",
    )
    parser.add_argument(
        "--sss_inner_rank",
        type=int,
        default=None,
        help="Inner subspace rank for SSS-like projection (default: enc_in//2). Used when spatial_mode=sss.",
    )
