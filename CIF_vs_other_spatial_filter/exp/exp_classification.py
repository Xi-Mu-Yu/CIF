...
from utils.csp import fit_csp_filters_from_batches
from utils.mne_spatial import (
    fit_ica_matrix_from_batches,
    fit_ssp_matrix_from_batches,
    fit_sss_matrix_from_batches,
)
...
class Exp_Classification(Exp_Basic):
    ...
    def train(self, setting,logger):
        ...
        if self.args.model == "TCN":
            spatial_mode = getattr(self.args, "spatial_mode", "cif")
            sfreq = getattr(self.args, "sampling_rate", 256)
            core = (
                self.model.module
            ...
            if spatial_mode == "csp":
                logger.info(
                    "Fitting CSP filters with mne.decoding.CSP "
                    "(two-group: smallest label vs rest)..."
                )
                W_np = fit_csp_filters_from_batches(
                    train_loader,
                    self.args.enc_in,
                    reg=getattr(self.args, "csp_reg", 1e-6),
                )
                core.set_spatial_matrix(torch.from_numpy(W_np).float())
                logger.info("CSP spatial filters installed on TCN.")
            elif spatial_mode == "ica":
                logger.info(
                    "Fitting ICA with MNE (exclude %d high-kurtosis components)...",
                    getattr(self.args, "ica_n_exclude", 1),
                )
                W_np = fit_ica_matrix_from_batches(
                    train_loader,
                    self.args.enc_in,
                    sfreq=sfreq,
                    n_components=getattr(self.args, "ica_n_components", None),
                    n_exclude=getattr(self.args, "ica_n_exclude", 1),
                )
                core.set_spatial_matrix(torch.from_numpy(W_np).float())
                logger.info("ICA spatial matrix installed on TCN.")
            elif spatial_mode == "ssp":
                logger.info(
                    "Fitting SSP projectors with MNE (n_proj=%d)...",
                    getattr(self.args, "ssp_n_proj", 1),
                )
                W_np = fit_ssp_matrix_from_batches(
                    train_loader,
                    self.args.enc_in,
                    sfreq=sfreq,
                    n_proj=getattr(self.args, "ssp_n_proj", 1),
                )
                core.set_spatial_matrix(torch.from_numpy(W_np).float())
                logger.info("SSP spatial matrix installed on TCN.")
            elif spatial_mode == "sss":
                logger.info(
                    "Fitting SSS-like inner subspace (rank=%s)...",
                    getattr(self.args, "sss_inner_rank", None),
                )
                W_np = fit_sss_matrix_from_batches(
                    train_loader,
                    self.args.enc_in,
                    inner_rank=getattr(self.args, "sss_inner_rank", None),
                )
                core.set_spatial_matrix(torch.from_numpy(W_np).float())
                logger.info("SSS spatial matrix installed on TCN.")
