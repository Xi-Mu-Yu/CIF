...
class ShallowNetEmbedding(nn.Module):
    def __init__(self, c_in, d_model, dropout):
        ...
        self.projection = nn.Sequential(nn.Conv2d(d_model, d_model, (1, 1), stride=(1, 1)))
...
class UpDimensionChannelEmbedding(nn.Module):
...
class TokenChannelEmbedding(nn.Module):
    def __init__(
        ...
        self.enc_in = enc_in
        self.paddings = [nn.ReplicationPad1d((0, stride)) for stride in stride_list]
        self.value_embeddings_t = nn.ModuleList(
            [
                CrossChannelTokenEmbedding(c_in=enc_in, l_patch=patch_len, d_model=d_model)
                for patch_len in patch_len_list
            ]
        )
        self.value_embeddings_c = nn.ModuleList(
            [
                UpDimensionChannelEmbedding(
                    c_in=enc_in, t_in=seq_len, u_dim=u_dim, d_model=d_model
                )
    ...
    def forward(self, x):
        ...
        for padding, value_embedding_t in zip(self.paddings, self.value_embeddings_t):
            ...
            x_new_t = padding(x_new_t).unsqueeze(1)
            x_new_t = value_embedding_t(x_new_t)
            x_new_t = x_new_t.squeeze(2).transpose(1, 2)
        ...
        for value_embedding_c in self.value_embeddings_c:
            ...
            x_new_c = value_embedding_c(x_new_c)
            x_list_c.append(x_new_c)
        x_t = [
            x + cxt + self.position_embedding_t(x)
            for x, cxt in zip(x_list_t, self.learnable_embeddings_t)
        ]
        x_c = [x + cxt for x, cxt in zip(x_list_c, self.learnable_embeddings_c)]
        return x_t, x_c


class BIOTEmbedding(nn.Module):
    def __init__(self, enc_in, seq_len, d_model, patch_len, stride, augmentation=["none"]):
        super().__init__()
        self.patch_len = patch_len
        self.stride = stride
    ...
    def forward(self, x):
        ...
        x = self.padding(x)
        x = x.unfold(dimension=-1, size=self.patch_len, step=self.stride)
        x = rearrange(x, "b e n l -> (b e) n l")
        x = self.value_embedding(x) + self.position_embedding(x)
        x = rearrange(x, "(b e) n d -> b (e n) d", e=enc_in)
