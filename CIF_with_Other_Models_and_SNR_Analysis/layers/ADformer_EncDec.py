...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x_t, x_c, attn_mask=None, tau=None, delta=None):
        new_x_t, new_x_c, attn_t, attn_c = self.attention(
            x_t, x_c, attn_mask=attn_mask, tau=tau, delta=delta
        )
...
class Encoder(nn.Module):
    ...
    def forward(self, x_t, x_c, attn_mask=None, tau=None, delta=None):
        ...
        for attn_layer in self.attn_layers:
            x_t, x_c, attn_t, attn_c = attn_layer(
                x_t, x_c, attn_mask=attn_mask, tau=tau, delta=delta
            )
        ...
        if x_t:
            x_t = [x[:, -1, :].unsqueeze(1) for x in x_t]
        else:
            x_t = None
        if x_c:
            x_c = [x[:, -1, :].unsqueeze(1) for x in x_c]
        else:
            x_c = None
