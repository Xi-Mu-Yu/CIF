...
class Model(nn.Module):
    def __init__(self, configs, hidden_dims=128, output_dims=320, kernel_size=3):
        ...
        self.t = configs.t
        self.n = configs.n
        self.log_snr = bool(getattr(configs, "log_snr", False))
        self.snr_log_interval = max(1, int(getattr(configs, "snr_log_interval", 100)))
        self._forward_steps = 0
        self.last_snr_before_db = None
        self.last_snr_after_db = None
        # self.a = configs.a
        # self.b = configs.b
    ...
    def _trial_avg_snr_db(self, x):
        """
        Trial-averaging SNR estimator:
        signal = batch mean (phase-locked component),
        noise = residual from each trial to the batch mean.
        x shape: [B, T, C]
        """
        if x.size(0) < 2:
    ...
    def _trial_avg_snr_db_per_channel(self, x):
        """
        Per-channel trial-averaging SNR estimator.
        x shape: [B, T, C] -> returns [C]
        """
        if x.size(0) < 2:
            # Keep dtype/device consistent for easy printing.
            return torch.full((x.size(-1),), float("nan"), device=x.device)

        # signal: [1, T, C], noise: [B, T, C]
        signal = x.mean(dim=0, keepdim=True)
        noise = x - signal

        # Average power across time; signal has batch dim=1.
        signal_power = signal.pow(2).mean(dim=(0, 1))  # [C]
        noise_power = noise.pow(2).mean(dim=(0, 1)).clamp_min(1e-12)  # [C]
        return 10.0 * torch.log10(signal_power / noise_power)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        n = self.n
        front_8_half = x_enc[:, :, 6:7]  # 取序列后半部分 & 前8个通道
        back_8_half = x_enc[:, :, 7:8]  # 取序列后半部分 & 后8个通道
        x_enc_new = x_enc.clone()  # 深拷贝原张量，避免修改原数据


        # print("a_value-------------",self.a)
        # print("b_value-------------",self.b)

        # added_half = front_8_half * -0.32 + back_8_half * 0.4
        added_half = front_8_half * self.a + back_8_half * self.b   # ADFTD
        # added_half = front_8_half * self.coefficients[0] + back_8_half * self.coefficients[1]
        # added_half = front_8_half*0.2  + back_8_half *-0.6
        
        if t >0:
            x_enc_new[:, :, 6:7] = added_half  # 替换前 8 通道的后半部分
        else:
            x_enc_new[:, :, 7:8] = added_half 
        # x_enc_new= torch.matmul(x_enc, self.channel_weights)

        if self.log_snr:
            with torch.no_grad():
                snr_before_db = self._trial_avg_snr_db(x_enc)
                snr_after_db = self._trial_avg_snr_db(x_enc_new)
                snr_before_db_per_ch = self._trial_avg_snr_db_per_channel(x_enc)
                snr_after_db_per_ch = self._trial_avg_snr_db_per_channel(x_enc_new)
                self.last_snr_before_db = float(snr_before_db.detach().cpu())
                self.last_snr_after_db = float(snr_after_db.detach().cpu())
                self._forward_steps += 1
                if self._forward_steps % self.snr_log_interval == 0:
                    print(
                        f"[SNR] step={self._forward_steps} "
                        f"before={self.last_snr_before_db:.4f} dB "
                        f"after={self.last_snr_after_db:.4f} dB "
                        f"delta={self.last_snr_after_db - self.last_snr_before_db:.4f} dB"
                    )
                    delta_per_ch = (snr_after_db_per_ch - snr_before_db_per_ch).detach().cpu().tolist()
                    delta_str = ", ".join([f"ch{i}:{d:+.4f}dB" for i, d in enumerate(delta_per_ch)])
                    print(f"[SNR/CH] step={self._forward_steps} delta={delta_str}")
